...
class ChatCameraScreen extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      groupProfilePicture: props.navigation.state.params.profilePicture,
      fontsLoaded: false,
      showAct: false,
      // onPressCamera: false,
  ...
  uploadVideo = async () => {
    console.log('a intrat in upload video:', this.state.captures[0].uri)
    const timestamp = firebase.auth().currentUser.uid + "/" + Date.now()
    const path = `videos/${this.state.roomId}/${timestamp}`
  ...
  uploadPhoto = async () => {
    console.log('a intrat in upload phote')
    const timestamp = firebase.auth().currentUser.uid + "/" + Date.now()
    const path = `photos/${this.state.roomId}/${timestamp}`
    const response = await fetch(this.state.captures[0].uri)
    const file = await response.blob()
    let upload = firebase.storage().ref(path).put(file)
    upload.on("state_changed", snapshot => { }, err => {
  ...
  upload = (url) => {
    console.log('a intrat in send photo')
  ...
  sendPost = (url) => {
    console.log('a intrat in send post')
    firebase.firestore().collection('messages').doc(this.state.roomId).collection('chats').add({
      mood: this.state.mood,
      msg: '',
      activity: this.state.selectedActivity,
...
export default withNavigation(ChatCameraScreen)